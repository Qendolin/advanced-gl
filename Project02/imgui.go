...
func NewImGui(vshSrc, fshSrc string) *ImGui {
	...
	var shader uint32
	gl.CreateProgramPipelines(1, &shader)

	cStrs, free := gl.Strs(vshSrc + "\x00")
	vertSh := gl.CreateShaderProgramv(uint32(gl.VERTEX_SHADER), 1, cStrs)
	free()

	var ok int32
	if gl.GetProgramiv(vertSh, gl.LINK_STATUS, &ok); ok == gl.FALSE {
		var logLength int32
		gl.GetProgramiv(vertSh, gl.INFO_LOG_LENGTH, &logLength)

		errorLog := strings.Repeat("\x00", int(logLength+1))
		gl.GetProgramInfoLog(vertSh, logLength, nil, gl.Str(errorLog))
		log.Panicf("failed to link imgui vertex shader, log: %v", errorLog)
	}
	gl.ValidateProgram(vertSh)
	if gl.GetProgramiv(vertSh, gl.VALIDATE_STATUS, &ok); ok == gl.FALSE {
		var logLength int32
		gl.GetProgramiv(vertSh, gl.INFO_LOG_LENGTH, &logLength)

		errorLog := strings.Repeat("\x00", int(logLength+1))
		gl.GetProgramInfoLog(vertSh, logLength, nil, gl.Str(errorLog))
		log.Panicf("failed to validate imgui vertex shader, log: %v", errorLog)
	}
	gl.UseProgramStages(shader, gl.VERTEX_SHADER_BIT, vertSh)

	cStrs, free = gl.Strs(fshSrc + "\x00")
	fragSh := gl.CreateShaderProgramv(uint32(gl.FRAGMENT_SHADER), 1, cStrs)
	free()

	if gl.GetProgramiv(fragSh, gl.LINK_STATUS, &ok); ok == gl.FALSE {
		var logLength int32
		gl.GetProgramiv(fragSh, gl.INFO_LOG_LENGTH, &logLength)

		errorLog := strings.Repeat("\x00", int(logLength+1))
		gl.GetProgramInfoLog(fragSh, logLength, nil, gl.Str(errorLog))
		log.Panicf("failed to link imgui fragment shader, log: %v", errorLog)
	}
	gl.ValidateProgram(fragSh)
	if gl.GetProgramiv(fragSh, gl.VALIDATE_STATUS, &ok); ok == gl.FALSE {
		var logLength int32
		gl.GetProgramiv(fragSh, gl.INFO_LOG_LENGTH, &logLength)

		errorLog := strings.Repeat("\x00", int(logLength+1))
		gl.GetProgramInfoLog(fragSh, logLength, nil, gl.Str(errorLog))
		log.Panicf("failed to validate imgui fragment shader, log: %v", errorLog)
	}
	gl.UseProgramStages(shader, gl.FRAGMENT_SHADER_BIT, fragSh)
	...
	return &ImGui{
		...
		shader:    shader,
		vertSh:    vertSh,
		uProjMat:  gl.GetUniformLocation(vertSh, gl.Str("u_proj_mat\x00")),
	}
}
...
func (gui *ImGui) Draw() {
	...
	gl.BindProgramPipeline(gui.shader)
	gl.ProgramUniformMatrix4fv(gui.vertSh, gui.uProjMat, 1, false, &ortho[0])
